...
package struct GraphQLTypesGenerator {
    ...
    package func generate(schema: GraphQLSchema) throws -> String {
        ...
        for type in unionTypes {
            ...
            let swiftTypeName = try swiftTypeDeclaration(
                for: type,
        ...
        let objectTypes = types.compactMap {
            ...
            $0.name != schema.queryType?.name && $0.name != schema.mutationType?.name
    ...
    func generateInputStruct(for type: GraphQLInputObjectType) throws -> String {
        ...
        for (fieldName, field) in fields {
            ...
            let returnType = try swiftTypeReference(
                for: field.type,
                includeNamespace: false,
                nameGenerator: nameGenerator
            )
    ...
    func generateInterfaceProtocol(for type: GraphQLInterfaceType) throws -> String {
        ...
        for (fieldName, field) in fields {
            ...
            for (argName, arg) in field.args {
                let argType = try swiftTypeReference(
                    for: arg.type,
                    includeNamespace: false,
                    nameGenerator: nameGenerator
                )
    ...
    func generateTypeProtocol(
        for type: GraphQLObjectType,
        unionTypeMap: [String: [GraphQLUnionType]]
    ) throws -> String {
        var output = ""
        ...
        let unions =
            try unionTypeMap[type.name]?.map {
                try swiftTypeDeclaration(
                    for: $0,
                    includeNamespace: false,
                    nameGenerator: nameGenerator
                ) + ", "
    ...
    func generateRootTypeProtocol(for type: GraphQLObjectType) throws -> String {
        ...
        for (fieldName, field) in fields {
            ...
            var returnType = try swiftTypeReference(
                for: field.type,
                includeNamespace: false,
                nameGenerator: nameGenerator
            )
